...
plot(1:noPeople,result[1,], xaxt="n",type="b",xlab="Person",ylab="Success Rate",ylim=(c(min(min(result),min(result_b)),max(max(result),max(result_b)))),col=colors[2]) 
lines(1:noPeople,result_b[1,], type="b",col=colors[1]) 
legend("topright", legend=c("z-score","entropy"),cex=0.8,col=rev(colors), lty=array(1,2))
