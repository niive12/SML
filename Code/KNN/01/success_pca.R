...
if ( file.exists(fileName) && 1 ) {
	...
	if( 1 ) {
		people_data = prepareOneAlone(3,1) #9 people vs 1 person             
...
plot(acc,one_success,type="b",lty=16, xlab="PC",ylab="variance",ylim=(c(min(c(one_success,mix_success)),max(c(one_success,mix_success)))),col=colors[1])
lines(acc,mix_success,type="b",lty=15, col=colors[2])
axis(1, at=acc)
legend("bottomright",NULL,c("all vs one","all mixed"),cex=0.8,col=colors,lty=16:15)
quiet = dev.off()