...
The tree was built with the minimum number of 
...
It can be seen that after a few trials the model is has a 100\% success rate on the training data.
...
Since a full data set is having 400 PC on the data it can be seen that \textbf{a combination of 130 PC and 5 trials} gives the best result.
\todo[inline]{update numbers when you have the figure}

\begin{figure}[h]
\missingfigure{}
% \includegraphics[width=\textwidth]{graphics/tree_pca_boost}
...
\begin{figure}[h]
\missingfigure{}
% \includegraphics[width=\textwidth]{graphics/tree_pca_boost_timing}
\caption{Time of boosting the tree vs number of PC used. Measured in seconds}
...
This was tested 10 times so all data was used for testing at one test. 
